...
class FFT(nn.Module):
    ...
    def forward(self, input, target):
        fft1 = torch.rfft(input, signal_ndim=2, normalized=False, onesided=False)
        fft2 = torch.rfft(target, signal_ndim=2, normalized=False, onesided=False)
        fft = F.l1_loss(fft1, fft2)
